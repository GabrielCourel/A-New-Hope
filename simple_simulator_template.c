			...
			switch(opcode){
				...
				case PUSH:
                    ...
                    if(pega_pedaco(IR,6,6)==0)
                        ...
                        selM3=8; //pra passar o flag register
                    }
                    selM5=sM3;
                    DecSP=1;
					// -----------------------------
...
// Funcao que separa somente o campo de interesse para executar uma instrução, opcode ou identificar o registrador de destino
int pega_pedaco(int ir, int a, int b) {
	int pedaco=0;

	//essas operações geram um número binário com N 1s, onde N é o tamanho do campo calculado por a-b+1

	pedaco = ((pow(2,(a-b+1)))-1);
	//posicionamento da mascara que move os 1s para posição correta após retirar os números fora do campo
	pedaco = pedaco & ir >> b;
	//valor retornado é o número inteiro que representa o código binário (Opcode, Registrador, etc.) que estava originalmente entre os bits "a" e "b"
